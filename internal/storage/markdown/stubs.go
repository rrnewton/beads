...
// GetDirtyIssueHash returns empty string for markdown storage (no persistent hash tracking)
func (m *MarkdownStorage) GetDirtyIssueHash(ctx context.Context, issueID string) (string, error) {
	// Markdown storage doesn't track content hashes - always export dirty issues
	return "", nil
}

// GetExportHash returns empty string for markdown storage (no persistent export hash tracking)
func (m *MarkdownStorage) GetExportHash(ctx context.Context, issueID string) (string, error) {
	// Markdown storage doesn't track export hashes - export tracking is not needed
	return "", nil
}

// SetExportHash is a no-op for markdown storage (no persistent export hash tracking)
func (m *MarkdownStorage) SetExportHash(ctx context.Context, issueID, contentHash string) error {
	// Markdown storage doesn't track export hashes - no-op
	return nil
}
